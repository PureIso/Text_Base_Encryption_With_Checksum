...
namespace TextBaseEncryption
{
    ...
    {
        #region Variables
        private readonly byte[] _key = { 0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 
        ...
        private static readonly uint[] CrcTable = new uint[]
                                                      {
                                                          ...
                                                          0xb6662d3d,
                                                          0x76dc4190, 0x1db7106, 0x98d220bc, 0xefd5102a, 0x71b18589,
                                                          0x6b6b51f, 0x9fbfe4a5, 0xe8b8d433, 0x7807c9a2, 0xf00f934,
                                                          0x9609a88e, 0xe10e9818, 0x7f6a0dbb, 0x86d3d2d, 0x91646c97,
                                                          0xe6635c01,
                                                          ...
                                                          0xc0ba6cad,
                                                          0xedb88320, 0x9abfb3b6, 0x3b6e20c, 0x74b1d29a, 0xead54739,
                                                          0x9dd277af, 0x4db2615, 0x73dc1683, 0xe3630b12, 0x94643b84,
                                                          ...
                                                          0xeb0e363f, 0x72076785, 0x5005713, 0x95bf4a82, 0xe2b87a14,
                                                          0x7bb12bae, 0xcb61b38, 0x92d28e9b, 0xe5d5be0d, 0x7cdcefb7,
                                                          0xbdbdf21,
        ...
        private void encryptButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                byte[] buffer = System.Text.Encoding.UTF8.GetBytes(passwordTextBox.Text);
                //Copy to key
                Array.Copy(buffer, 0, _key, 0, buffer.Length);
                //Turn Data to byte[]
                byte[] data = System.Text.Encoding.UTF8.GetBytes(dataTextBox.Text);
                ...
                using (MemoryStream memoryStream = new MemoryStream())
                {
                    ...
                    using (CryptoStream crytoStream = new CryptoStream(memoryStream, tripleDes.CreateEncryptor(), CryptoStreamMode.Write))
                    {
                        ...
                        string command = checksumComboBox.GetItemText(checksumComboBox.SelectedItem);
                        switch (command)
                        {
                            case "CRC 32":
                                Array.Copy(ComputeCrc32(cipherBytes), 0, newBuffer, cipherBytes.Length, 4);
                                break;
                            case "Adler 32":
                                Array.Copy(ComputeAdler32(cipherBytes), 0, newBuffer, cipherBytes.Length, 4);
                                break;
                            case "CRC 32 Jam":
                                Array.Copy(ComputeCrc32Jam(cipherBytes), 0, newBuffer, cipherBytes.Length, 4);
                                break;
                            default:
                                throw new Exception("Select a Checksum");
                        }
                ...
                dataTextBox.Text = encryptedData;
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "Text Based Encryption", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void decryptButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                byte[] encryptedData = Convert.FromBase64String(dataTextBox.Text);
                //Get actual data
                ...
                {
                    case "CRC 32":
                        check = ComputeCrc32(realData);
                        break;
                    case "Adler 32":
                        check = ComputeAdler32(realData);
                        break;
                    case "CRC 32 Jam":
                        check = ComputeCrc32Jam(realData);
                        break;
                    default:
                        throw new Exception("Select a Checksum");
                }
                ...
                dataTextBox.Text = Encoding.UTF8.GetString(resultArray);
        ...
        #region Checksum Function
        /// <summary>Calculate CRC32 using CRC Tables</summary>
        /// <param name="value"> The value that will be calculated</param>
        /// <returns>Calculated Bytes</returns>
        private static byte[] ComputeCrc32(byte[] value)
        {
        ...
        private static byte[] ComputeCrc32Jam(byte[] buffer)
        {
            uint[] crcTable = new uint[256];
            const uint crcInitialValue = 0XFFFFFFFFU;

            uint tmp = crcInitialValue;
            foreach (byte b in buffer)
            {
                tmp = (tmp >> 8) ^ crcTable[Convert.ToUInt32(tmp ^ b) & 0XFF];
            }
            buffer = BitConverter.GetBytes(Convert.ToUInt32(tmp));
            Array.Reverse(buffer);
            return buffer;
        }

        private static byte[] ComputeAdler32(byte[] buffer)
        {
            ...
            byte[] output = BitConverter.GetBytes(Convert.ToUInt32((b << 16) | a));
